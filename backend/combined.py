...
def extract_audio_features(
    ...
    try:
        # Get pitch (fundamental frequency) contour
        pitch = snd.to_pitch(time_step=time_step, pitch_floor=pitch_floor, pitch_ceiling=pitch_ceiling)

        # Get PointProcess (cross-correlation) from sound, pitch_floor, pitch_ceiling
...
# === Load CSV with test cases ===
df = pd.read_csv("test_cases.csv")

# Separate audio features and blink rate
...
# Predict and combine
i=len(df)-1
audio_proba = audio_model.predict_proba([audio_scaled[i]])[0][1]
blink_proba = blink_model.predict_proba([[blink_rates[i]]])[0][1]
age_proba = age_model.predict_proba([[df['Age'][i]]])[0][1]
...
fused_prediction = 1 if fused_proba >= 0.5 else 0
...
print(f" - Final Prediction: {'Parkinson\'s Detected (1)' if fused_prediction == 1 else 'No Parkinson\'s (0)'}")
